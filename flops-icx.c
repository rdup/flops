...
double nulltime;
//double TimeArray[3];   /* Variables needed for 'dtime()'.     */
...
FILE* lixo = fopen("/dev/null", "w");
   ...
   printf("  modified for Intel(R) oneAPI DPC++/C++ Compiler 2023.1.0 \n\n");
...
/*************************/
   ...
   dtime(lixo,&TimeArray1,&TimeArray2);
   dtime(lixo,&TimeArray1,&TimeArray2);
...
int dtime(FILE *lixo,double *p1,  double *p2)
//double p[3];
{
   double q,pp1, pp2;
   char st[80];
   ...
   pp2 = (double)(rusage.ru_utime.tv_sec);

   pp2 = pp2 + (double)(rusage.ru_utime.tv_usec) * 1.0e-06;
   pp1 = pp2 - q;
   *p1=pp1;
   *p2=pp2;
   fprintf(lixo,"p2 = %lf \n", *p2); 
...
/*------------- End flops.c code, say good night Carol! --------------*/
